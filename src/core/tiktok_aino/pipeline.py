...
def _reference_unique_post_title(hook: str, topic: TopicCandidate, sources: dict[str, Source] | None) -> str:
    ...
    if "김건희" in topic_text:
        return _compact_card_text("김건희 수사, 빈칸은?", 24, keep_question=True)
    if "검찰" in topic_text:
        return _compact_card_text("검찰 발언, 기록은?", 24, keep_question=True)
    if "조국" in topic_text:
        return _compact_card_text("조국 낙선, 원인은?", 24, keep_question=True)
    if primary:
        return _compact_card_text(f"{primary}, 기록은?", 24, keep_question=True)
...
def apply_reference_content_design(
    ...
    rewritten = _script_with_scenes(
        ...
        title=post_title,
        post_title=post_title,
        pinned_comment=f"{comment_question} 근거 기준으로 댓글 남겨주세요.",
    )
    return _enrich_post_metadata(rewritten, topic, sources)
