...
## 6. Veo 영상 엔진 결정 (owner "투입단가 대비 수익화로 직접 판단" 위임, 2026-06-12)

**검증된 사실**: veo_automation.py 가동 가능 (Vertex AI 활성 project ethereal-cache-487709-s3, 예산 가드 내장 monthly_cap/run_cap/ledger). leftaino 정치 영상은 그동안 Veo 미사용 = 정적 Gemini 이미지 슬라이드쇼(카메라 모션 0) → 양산형의 근본.

**ROI 분석 (Claude 판단)**:
- R2 직접 수익(Creator Rewards)은 팔로워 10K 선행 → **현재 영상의 수익 = $0**. 따라서 영상의 목적 = 팔로워 전환(retention/follow), 수익은 후행.
- 병목 = 팔로워 성장 = 게시 볼륨 × 편당 전환력. 정적 이미지(싸지만 양산형)는 전환 실패 입증됨 = **최악 ROI** (생성 노력 낭비).
- 풀 Veo(영상당 $5~8)는 $0 수익 단계에서 과투자.

**결정 = 하이브리드 (Veo fast 훅·핵심비트 + 무료 키네틱 카드 리빌)**:
- 훅(첫 1~2초, 스크롤 정지 = 최고 레버리지) + 정서 핵심 비트 1~2개 = **Veo fast** ($0.08~0.12/초, 클립당 4~6초).
- 랭킹 숫자 리빌 = 무료 키네틱 카드 (숫자·텍스트가 곧 콘텐츠, 모션 불필요 — W8 산출 재활용).
- **영상당 ~$1~1.5** (Veo 클립 2~3개). 월 20~30 hero 영상 = 볼륨 유지.
- **Veo 월 캡 = $35 (~₩50K)**, Gemini 런타임 ~₩50K → ₩10만 총량 내. W4 페이서가 Vertex Veo 지출 합산 추적.
- **킬스위치**: 전환 측정 윈도(예: hero 10편) 후 follow-growth 개선 無 → lite 강등 또는 순수 키네틱 회귀. $0 수익 단계 성장 투자로 관리, 회수 신호 생기면 재평가.

**근거 한 줄**: 수익 0 단계에선 "전환되는 콘텐츠를 최소 단가로" = 훅에만 모션 투입 + 구조적 리빌은 무료 디자인. 풀 Veo도 순수 정적도 ROI 열위.


## 7. 정정 (owner 2026-06-12 14:28) — 엔진 매핑 오류 수정

**Claude 오매핑**: leftaino(정치)에 Veo를 붙이려 함(W12) = 틀림. owner 의도:
- **leftaino(정치 틱톡) = 이미지 개선** (Veo 아님). → W8 키네틱 타이포 카드가 정답. **확정·유지** (커밋됨). Veo 미적용.
- **ROOM707 = Veo 격상**. 현재 LTX 2B 로컬 → owner는 Veo 품질 원함. EP003+ 대상 (EP002는 LTX로 이미 완성·업로드 중).

**조치**: W12(leftaino+Veo) 중단·되돌림 완료. §6 Veo ROI 결정은 **707에 적용**으로 이관 (정치 틱톡 아님). leftaino 비주얼은 W8 카드로 종결, 추가 Veo 투입 없음.
