...
## ① EP003 0뷰 진단 결론 (Studio 애널리틱스 실측 기반, 6/14 16:49)
**섀도우밴/계정제한 아님.** Studio 개요(지난 7일): 동영상 조회수 **422**, 트래픽 소스 **추천(For You) 98.1%** / 검색 0% / 팔로잉 0%. 422 = EP001 327 + EP002 95 + EP003 0 + EP004 0 (정확 합치). 콘텐츠 목록에 "For You 권장 대상 아님" 배너 없음. → **For You는 정상 분배 중인데 최근 영상만 0.**
**진짜 원인 = 리텐션/인게이지먼트 붕괴 throttle.** 327→95(−71%)→0→0 단조 감소. EP001이 콜드스타트 테스트(327)를 받았으나 전환 약함(9♥/2.75%, 댓글1, 공유0) → 알고리즘이 EP002 테스트를 95로 축소 → EP002 더 약함(0♥) → EP003/EP004는 테스트 자체가 ~0. = 전략문서 확정사실 #1 "완주율 죽은구간"의 실측 확증. **에피소드 수·밴이 아니라 완주율/초반 인게이지먼트가 레버.**
구조적 가중: 한국 리전 계정 + 영어 글로벌 콘텐츠 → 1차 테스트 시청자(한국) 미스매치 → 첫 임프레션부터 완주율 낮음. EP003 일회성 가중: 게시 mess(SET_PUBLIC_FAILED로 초반 푸시창 동안 비공개/검토중 추정) + 공식 AI라벨 미표시.
미측정 1건: 영상별 완료율/평균시청시간 (TikTok 딥링크 Access Denied → 앱내 수동 확인만, `gate_track.py --completion` 주입용).
