# ⭐ 2026-06 현재 상태 (CURRENT — 이 entry가 최신, 아래 4~5월 entries는 역사 기록) ⭐

> **박제:** 2026-06-12 Claude Fable 5 (회사PC 원격 세션) — owner "집pc에서는 파악 못하고 있는거 같은데" 지적에 따른 SSOT 동기화
> **정본 위치:** 이 디렉토리 (`D:\00.test\001.ssot-agent-runtime\002.neo-genesis-sbu-autogrowth\.agent\`) 가 현행 SSOT. 구 `D:\00.test\neo-genesis\.agent\` 는 폐기 (마스터 크레덴셜 `.env.local`만 그곳 유지).

## 회사 구조 (2026-06 재편 후)
- **제품 = `D:\00.test\002.products-sbu\` 잔존 5개**: 002.landing-page / 005.sora-app / 011.room-707(6/25 게이트) / **012.finite (주력)** / 013.ait
- **Kill 집행 완료 (6/12)**: quant-bot 포함 사망 판정 9개 레포 → `D:\00.test\009.archive\products-killed-20260611\` (MANIFEST.md에 원복 명령, 가역). **퀀트 v11은 사망 판정** — 아래 quant entries는 역사 기록.
- 11-SBU/Sora-enterprise/RAG-24주 서사는 4~5월 체제 — 현행 우선순위 아님.

## 주력 제품: 012.finite (DaysLeft, daysleft.io)
- **90일 KPI = 첫 결제 1건. 신규 제품 착수 금지 판정.**
- **6/12 결제 라이브**: PR #2 머지 (main `a7bca1d`) → Vercel 프로덕션 → E2E 검증 PASS. Polar.sh MoR (상품 5 SKU + benefit 3 + 체크아웃 5링크 + Vercel env 10키). 구매 흐름: /upgrade → buy.polar.sh → /activate 라이선스 활성화 → Pro/Plus 게이팅 해제.
- Free funnel(측정/재측정/델타/공유) 무료 유지 + gentle/PHQ 안전 경로 업셀 미노출 (owner 제품 원칙).
- 가격 정본 = finite repo `apps/web/lib/plans.ts` / env 키 정본 = `apps/web/.env.example`.
- owner 잔여: POLAR_ACCESS_TOKEN 회전(채팅 노출 1회), Polar KYC·정산계좌(첫 정산 전), 결제 이벤트 계측(리텐션 WIP 머지 후).

## 계측 (6/11~12 플릿 정비 완료)
- PostHog 프로젝트 **322404 단일** (`$host` 필터 필수). 합성 트래픽 `$lib=neo-sbu-direct` 오염 → **13개 사이트 전부 게이트 적용으로 차단 완료**.
- 교훈: **집PC 로컬 레포 미러는 stale — GitHub(Yesol-Pilot)가 라이브 소스.** ethicaai/whylab 라이브 소스는 neo-genesis repo `src/sbu/*/site/`.

## KOTT 수익화 MVP (2026-06-13 Codex 완료)
- `kott.kr`는 추천/검색 DB에서 **작품 기준 이번 달 OTT 결제 판단 + B2B 검색 의도 리포트 + 스폰서/제휴/데이터 직접 판매 카드결제** 흐름으로 전환 완료.
- 완료: `/planner`, `/report`, `/advertise`, `/deals`, `/api/lead`, `/api/affiliate`, `/api/checkout`, `/report/paid` 보정, Polar Checkout Link 6개 연결, 홈/작품/시청처/광고 구매 CTA 전환, sitemap/indexing queue 반영.
- 배포: `Yesol-Pilot/kott` main `17ba6e5`, Vercel `neogenesis-d82d2888/kott`, live `https://kott.kr` smoke PASS. `/api/checkout?offer=intent-snapshot`와 `/api/checkout?offer=sponsor-slot`는 Polar checkout으로 302.
- 다음 수익 액션: Polar API `payment_ready=false`, `organization_status=created` 확인됨. owner가 KYC/정산계좌/account review를 완료해야 실제 카드 승인/정산 가능. 이후 첫 결제 발생 시 리포트/스폰서/데이터 납품 및 세금계산서 프로세스 실행, 승인형 제휴 신청, checkout/lead/outbound 이벤트 계측 확인.

## UR WRONG paid audit funnel (2026-06-13 Codex 완료)
- `https://ur-wrong.com/audit`는 유료 AI Answer Quality Audit offer로 라이브: $99 Starter Audit, $490 Team Benchmark, Custom Vendor Shortlist.
- SPA home/header/footer에 `/audit` 진입점을 추가해 public verdict product에서 직접 수익 CTA로 이동 가능.
- Lead capture endpoint: `/api/audit-lead`. 현재 production은 `neogenesis.research@gmail.com` prefilled email fallback이며, Supabase table migration은 존재하지만 아직 적용/활성화하지 않음.
- Live Playwright 검증 완료: desktop/mobile 모두 `/audit`, `$99`, `AI audit` 노출, `/audit` form 노출, horizontal overflow 없음, POST smoke 200.
- 다음 수익 액션: owner payment account가 준비되면 Stripe/Polar checkout 또는 invoice link env를 붙이고 Starter Audit CTA를 결제 선행 흐름으로 전환.

## 검증된 수요 / 전략
- toolpick.dev 오가닉 125~150명/월 (90일 7배) + ChatGPT referral 14건 — 유일한 검증 수요.
- 전략 정본: `.agent/knowledge/20260611_BUSINESS_AUDIT_REVENUE_STRATEGY_v1.md` + `handoff.md` 최상단 entries (6월 작업 기록 전부 여기 누적).

## 에이전트 규칙 (현행)
- 등록/계정/사업자 상태는 SSOT 문구가 아니라 **실사 기록으로 검증** (6/11 owner 지적 — 사업자등록 보유 확정).
- 결제/자본/비가역 액션 = owner 게이트. SBU growth 자율 승인은 유지.
...
- [ ] **퀀트봇 VM cutover + runtime lease 활성화**  
  ...
  - 경로: `002.products-sbu/quant-bot/docs/RESILIENCE-v8.4-pc-down.md`, `002.products-sbu/quant-bot/src/core/runtime-coordinator.js`  
...
- [x] **v11 마스터 설계 문서화 완료** — 6 병렬 전문가 교차검증 (수학·HFT/MM·Stat Arb·리스크·ML/RL·이벤트알파) 수렴된 6-알파 앙상블 아키텍처
  📍 `D:/00.test/002.products-sbu/quant-bot/docs/v11-ensemble/`
    - `INDEX.md` (진입점)
    - `MASTER_DESIGN.md`
    - `ROADMAP.md`
...
# 2026-06-13 Codex Update — ROOM707

- `002.products-sbu/011.room-707`: global target rule locked into project docs and content agent prompt. EP003 pipeline schema normalized; EP003 manifest/SRT/text_bomb generated; EP003 full dry-run PASS; EP002 validate PASS via ffmpeg fallback. Paid Veo generation and TikTok posting were not executed.
# CURRENT - TikTok AiNo 예약 복구 완료 (2026-06-13, Codex)

- Status: completed.
- Owner issue: 하루 3개 x 3일 예약 규칙 위반, 양산형/상황 불일치 이미지 사용.
- Resolution: 2026-06-14, 2026-06-15, 2026-06-16 각 08:10/11:20/19:30 슬롯 총 9개를 TikTok Studio에 예약 완료. 모든 최종 예약은 `publish_ready=true` 산출물만 사용했고, TikTok Studio에서 예약 시간/본문 매칭을 확인했다.
- Final grid:
  - 6/14: 08:10 `50곳 부족`, 11:20 `투표용지 부족`, 19:30 `국조 5개 질문`
  - 6/15: 08:10 `서버 압수수색`, 11:20 `증거보전 3가지`, 19:30 `11시50분 이후`
  - 6/16: 08:10 `넘버링`, 11:20 `보관상자`, 19:30 `57% 여론`
- Guardrail now active: upload/schedule path blocks non-`publish_ready`, `local_pillow` fallback, procedural/kinetic draft images, `situation_fit<85`, or failed `visual_quality`.
- Note: `travel_ban_20260615_1120` was intentionally rejected because it did not pass gates; it was replaced by `evidence_chain_20260615_1120`.
