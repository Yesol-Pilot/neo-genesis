...
### 후속 (오늘 세션 외)
- [ ] **텔레그램 봇 fix (owner action)** — 봇 토큰 ✅ 유효, **owner 가 `@neogenesiscriptonbot` 에게 `/start` 1회 클릭 필요**. 그 후 getUpdates 로 chat_id 자동 갱신 → 일일 리포트 텔레그램 즉시 작동
- [ ] **Heap 메모리 누수 추적** — 90% (어제 박스권) → 95.48% (오늘 15분 만 재상승). Risk Officer 가 🟡 경고 자동 부여. 24h 더 추세 본 후 메모리 profile 또는 봇 재기동 결정
- [ ] **Liquidation Stream 실제 구현** + v6-live-runner wiring (Phase 0 Task 0.1, 다음 세션 1순위)
...
| 4. 텔레그램 봇 fix | ⚠️ **owner action 필요** (봇 `/start` 1회 클릭). 자율 fix 불가 | 즉시 |
