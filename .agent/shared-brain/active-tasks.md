...
### 알파 진행 (v11 6 알파) — Week #1 대비 변동 없음
- **A1 Liquidation Cascade**: 인프라 ✅ / 알파 로직 ❌
- **A2 Mean Reversion OU**: 코드 ❌
- **A3 Extreme Funding**: 인프라 ✅ / 알파 ❌
- **A4 Macro Event**: 코드 ❌
- **A5 Funding/Basis Harvest**: 인프라 ✅ / v11 알파 wiring ❌
- **A6 Alt MM**: 코드 ❌
- **결론**: **0/6 페이퍼 모드 14일 검증 가능** (Week #1 동일)
...
- 정책 8개 (`.agent/policies/rag_*.yaml` + `gitleaks-korean-rules.toml`)
- 마이그레이션 1개 (`.agent/migrations/rag_v2/001_initial.sql`)
- Python 모듈 9개 (`src/core/rag_v2/__init__.py + chunk_metadata.py + provenance_classifier.py` / `scripts/rag_v2/__init__.py + migrate + embedding + rerank + check_mecab + watchdog`)
