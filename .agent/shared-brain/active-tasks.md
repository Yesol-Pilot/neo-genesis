...
- [ ] **Liquidation 첫 row 도착 검증** — 3분 0건 (시장 조용 또는 stream URL 검증 필요). 다음 세션 24h 누적 결과 확인
...
### Auto-Progression 시스템 (2026-04-26 가동, PR #4 #5 MERGED)

**자동 운영 루프 (owner 수동 개입 최소화):**

1. **Risk Officer cron** — 매일 09:00 KST → 일일 리포트 텔레그램 발송 (alerts.length > 0 시)
2. **Phase Gate Monitor cron** — 매시간 정각 → Phase 0/1 게이트 자동 평가, 통과/임박 시 텔레그램 알림 (24h dedup)
3. **Liquidation Stream** — 24/7 가동, Binance forceOrder WS 실시간 → quant_liquidation_events Supabase 누적
4. **Phase 0 Gate #3 자동 진척** — liquidation 일 100+ 7일 카운트 자동
5. **Phase 0 전체 통과 → Phase 1 진입 권고 + 1000만원 입금 권고** 자동 텔레그램 알림
6. **PR auto-merge** (별도 owner action 필요) — `auto-merge.yml.pending` 로컬 보관, owner 가 PAT workflow scope 갱신 후 활성
